...
func _ready() -> void:
	...
	if proc_gen == null:
		var placeholder_env = environment.getPlaceholderTile()
		for x in range(BOARD_SIZE):
			for y in range(BOARD_SIZE):
				create_terrain_tile(placeholder_env, Vector2i(x,y))
	else:
		proc_gen.generate_world(create_terrain_tile, create_building_on_tile, board_id)
...
func create_terrain_tile(terrain : EnvTerrain, tile_pos : Vector2i) -> void:
	...
	if (tile_pos.x + tile_pos.y) % 2 == 1:
		darken_tile = 1	
	env_tilemap.set_cell(tile_pos, 0, environment.getTilePosbyEnv(terrain), darken_tile)
	board_matrix.add_tile(tile_pos, terrain)
...
# try to place placeable on tile
# Private function
func create_building_on_tile(building_data: BuildingData, tile_pos : Vector2i) -> void:
	var build : Building = Building.new_building(building_data.id_name) 
	build.z_index = tile_pos.y
	add_child(build)
	
	# MUST TRIGGER BEFORE ADDING (otherwise places self on board then can score against itself)
	board_matrix.add_placeable_to_tile(tile_pos, build)
	
	build.position = get_local_centre_of_tile(tile_pos)
	build.get_node("JiggleAnimation").play("jiggle")
