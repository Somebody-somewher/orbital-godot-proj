...
func _create_server():
	...
	_register_player(name_field.text, multiplayer.get_unique_id())
	
func _connect_client(addr = ""):
	var peer = ENetMultiplayerPeer.new()
	
	if ip_addr_field.text.is_empty():
		addr = ip
	else:
		addr = ip_addr_field.text
	var error = peer.create_client(addr, port)
	if (error != OK):
		curr_status = "cannot host: " + error
		return error
...
func _on_peer_connected(id : int)  -> void:
	...
	_register_player.rpc_id(id, name_field.text)
...
func _on_connected_to_server()  -> void:
	curr_status = name_field.text + " has connected to the server! :D"
	print(curr_status)
...
func _on_host_pressed() -> void:
	...
	print(name_field.text + " is waiting for Players! .o.")
